...
async function fetchExpenses(scope, period, budget) {
    ...
    const plan = (0, fanOut_1.planFanOut)(budget, scope.groupIds.length);
    const groupIds = scope.groupIds.slice(0, plan.take);
    const trimmed = plan.trimmed;
    const perQuery = plan.perQuery;
    const run = (q) => q.where("createdAt", ">=", from)
        .where("createdAt", "<=", to)
        ...
        .catch((err) => { console.error("fetchExpenses query failed", (err === null || err === void 0 ? void 0 : err.message) || err); return null; });
    ...
    for (const snap of snaps) {
        ...
        for (const doc of snap.docs) {
            ...
            if (gid !== null && !(0, aiScope_1.inScope)(scope, gid) && d.ownerId !== scope.uid)
                continue;
            const created = d.createdAt;
            const when = created && typeof created.toDate === "function" ? created.toDate() : null;
            if (!when)
