...
exports.logErrorDigest = (0, scheduler_1.onSchedule)(
...
{ schedule: "every 6 hours", timeZone: "UTC", retryCount: 0 }, async () => {
    ...
    const refs = head.map((g) => db.doc(`errorGroups/${(0, errorState_1.groupDocId)(g.key)}`));
    const snaps = refs.length > 0 ? await db.getAll(...refs) : [];
    const byKey = new Map();
    snaps.forEach((snap, i) => {
        const data = snap.data();
        if (data)
            byKey.set(head[i].key, data);
    });
    const groups = (0, errorState_1.joinState)(head, (k) => byKey.get(k) || null).map((g) => {
        return {
            status: g.status,
            recurred: g.recurred,
            count: g.count,
