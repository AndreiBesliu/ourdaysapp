 ...
 */
function fingerprint(message, context) {
 ...
 */
function groupErrors(rows, maxUrls = 4) {
    const byKey = new Map();
    for (const r of rows) {
        ...
        entry.g.count += 1;
        if (str(r.uid))
            entry.uids.add(str(r.uid));
        if (str(r.url))
            entry.urls.add(str(r.url));
        ...
        if (iso) {
            if (!entry.g.lastSeen || at > timeOf(entry.g.lastSeen))
                entry.g.lastSeen = iso;
            if (!entry.g.firstSeen || at < timeOf(entry.g.firstSeen))
                entry.g.firstSeen = iso;
        }
    }
...
//# sourceMappingURL=errorGrouping.js.map