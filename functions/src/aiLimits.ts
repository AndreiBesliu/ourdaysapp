...
export const DEFAULT_USER_DAILY_USD = 0.25;
...
export interface AiLimits {
  ...
  killSwitch: boolean;
  /**
   * Which inputs were refused and replaced. Present so the admin screen can SAY that a value was
   * rejected, instead of displaying a number nobody set and letting it read as configuration.
   */
  clamped: string[];
}

export interface RawAiLimits {
 ...
 */
export function clampAiLimits(raw: RawAiLimits | null | undefined): AiLimits {
  const r = raw || {};
  ...
  if (global === null) global = DEFAULT_GLOBAL_DAILY_USD;
  if (global > MAX_GLOBAL_DAILY_USD) {
    clamped.push(`globalDailyUsd: capped at ${MAX_GLOBAL_DAILY_USD}`);
    global = MAX_GLOBAL_DAILY_USD;
  }
