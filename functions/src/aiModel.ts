 ...
 */
export const UNKNOWN_MODEL_PRICING = Object.values(MODEL_PRICING).reduce(
  (worst, p) => ({ inPerM: Math.max(worst.inPerM, p.inPerM), outPerM: Math.max(worst.outPerM, p.outPerM) }),
  { inPerM: 0, outPerM: 0 },
);

export function priceUsd(model: string, inTokens: number, outTokens: number): number {
  const p = MODEL_PRICING[model] || UNKNOWN_MODEL_PRICING;
