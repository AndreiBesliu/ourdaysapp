 ...
 * meaning anything.
 */
function usageMeta(result: unknown): Record<string, unknown> | undefined {
  const r = result as { usageMetadata?: Record<string, unknown>; response?: { usageMetadata?: Record<string, unknown> } };
  return r?.usageMetadata || r?.response?.usageMetadata;
}
...
export function usageOf(result: unknown): Usage {
  ...
  return {
    ...
    completionTokens: num(meta?.candidatesTokenCount) + num(meta?.thoughtsTokenCount),
  };
}
...
export function textOf(result: unknown): string {
  const r = result as { text?: unknown; response?: { text?: unknown } };
  if (typeof r?.text === "string") return r.text;
  const legacy = r?.response?.text;
  if (typeof legacy === "function") {
    try {
      const out = (legacy as () => unknown).call(r.response);
