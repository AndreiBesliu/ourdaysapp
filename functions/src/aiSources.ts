...
export { maySee, isPendingInvite } from "./aiVisibility";
import { maySee, isPendingInvite } from "./aiVisibility";
...
export async function fetchEvents(scope: Scope, period: Period, budget: number): Promise<SourceResult<EventItem>> {
  ...
  const snaps = await Promise.all(queries.map((q) => q.get().catch((err) => {
    console.error("fetchEvents query failed", err?.message || err);
    return null;
  })));
  ...
  const snaps = await Promise.all(
    scope.groupIds.map((g) =>
      db.collection(`groups/${g}/messages`)
        ...
        .then((s) => ({ g, s }))
        .catch((err) => {
          console.error("fetchChat query failed", g, err?.message || err);
          return null;
        })
    )
  );
