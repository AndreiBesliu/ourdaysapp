 ...
 */
export const STAMPED_FROM_MS = Date.parse('2026-09-16T00:00:00.000Z');
 ...
 */
export function activityMs(value: unknown): number | null {
 ...
 */
export function nextRoundsWon(state: any, winnerUid: string | null): { P1: number; P2: number } | null {
  if (!winnerUid) return null;
  const players = state?.players || {};
  const key = winnerUid === players.P1 ? 'P1' : winnerUid === players.P2 ? 'P2' : null;
  if (!key) return null;
  const base = { P1: 0, P2: 0, ...(state?.roundsWon || {}) };
 ...
 */
function dayEndMs(day: unknown): number | null {
 ...
 */
export function lastActivityMs(game: any): number | null {
  const stamped = activityMs(game?.lastMoveAt);
  const base = stamped ?? (activityMs(game?.createdAt) === null ? null : STAMPED_FROM_MS);
  const day = dayEndMs(game?.date);
  if (base === null) return day;
  return day === null ? base : Math.max(base, day);
}

export type ExpiryRefusal =
  | 'server-owned'     // a Warlord battle: it has its own forfeit path
  | 'already-closed'   // the session has been formally ended
  | 'no-timestamp'     // nothing to measure from; leave it alone rather than guess
  | 'future-timestamp' // last touched AFTER now: a clock is wrong, and guessing would be worse
  | 'still-fresh';     // touched within the window
 ...
 * skipping everything.
 */
export function expiryRefusal(game: any, nowMs: number): ExpiryRefusal | null {
  if (game?.gameType === SERVER_OWNED_GAME) return 'server-owned';
  ...
  if (game?.finalized === true) return 'already-closed';
  ...
  // is suspicious.
  const stamped = activityMs(game?.lastMoveAt);
  if (stamped !== null && stamped - nowMs > FUTURE_SLACK_MS) return 'future-timestamp';

  const last = lastActivityMs(game);
  if (last === null) return 'no-timestamp';
  if (nowMs - last < IDLE_MS) return 'still-fresh';
  return null;
}

/** Convenience for the call sites that only care about the verdict. */
export function shouldExpire(game: any, nowMs: number): boolean {
 ...
 */
export function closedSessionFields(game: any, abandoned: boolean) {
  return {
    status: 'finished',
    winner: getSessionWinner(game),
    finalized: true,
    // Only ever written as `true`. The arcade list uses it to say a game was closed by the clock
    // rather than by a person, and the leaderboard ignores it entirely — it recomputes the winner
    // from the board either way, so a session that was genuinely won still counts, and a 0-0 one
    // credits nobody.
