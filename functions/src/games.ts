...
export const expireIdleGames = onSchedule(
  ...
  async () => {
    ...
    try {
      ...
      const due: FirebaseFirestore.QueryDocumentSnapshot[] = [];
      for (const d of snap.docs) {
        const refusal: ExpiryRefusal | null = expiryRefusal({ id: d.id, ...d.data() }, now);
        if (refusal === null) due.push(d);
        else skipped[refusal] = (skipped[refusal] || 0) + 1;
      }

      counts.due = due.length;

      for (const d of due.slice(0, PER_RUN)) {
        const game = { id: d.id, ...d.data() } as Record<string, unknown>;
        try {
