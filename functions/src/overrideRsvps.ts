...
export function overrideRsvps(
  ...
  if (p) for (const [k, v] of Object.entries(p)) if (typeof v === "string") out[k] = v;

  // The request, heard ONLY about the caller.
  const r = plainMap(requested);
  if (r && uid) {
    const mine = r[uid];
    if (typeof mine === "string" && (RSVP_ANSWERS as readonly string[]).includes(mine)) out[uid] = mine;
    else delete out[uid];
  }
