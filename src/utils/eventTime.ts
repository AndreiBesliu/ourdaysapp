...
export function isValidTime(t: unknown): t is WallTime {
 ...
 */
export function startInstant(ev: {
  date?: unknown; time?: unknown; timezone?: unknown;
}, fallbackZone = 'UTC'): number | null {
...
export function reminderInstant(ev: {
  date?: unknown; time?: unknown; timezone?: unknown; reminderMinutes?: unknown;
}, fallbackZone = 'UTC'): number | null {
...
export function displayTime(
  ev: { date?: unknown; time?: unknown; timezone?: unknown },
  readerZone: string,
 ...
 */
export function timeFieldsFor(time: string | null | undefined, zone: string): {
...
export function isValidDayOffset(n: unknown): n is number {
  return typeof n === 'number' && Number.isInteger(n) && n >= 0 && n <= MAX_SPAN_DAYS;
}
 ...
 */
export function dayPlus(day: string, n: number): string | null {
  const [y, m, d] = String(day).split('-').map(Number);
  if (![y, m, d].every(Number.isFinite) || !Number.isInteger(n)) return null;
...
export function spanOf(ev: { date?: unknown; endDayOffset?: unknown; endTime?: unknown }): {
  ...
  const endDay = dayPlus(startDay, offset) ?? startDay;
 ...
 */
export function occursOn(ev: { date?: unknown; endDayOffset?: unknown; endTime?: unknown }, day: string): boolean {
...
export function daysOf(ev: { date?: unknown; endDayOffset?: unknown; endTime?: unknown }): string[] {
  const span = spanOf(ev);
  if (!span) return [];
  const out: string[] = [];
 ...
 */
export function endInstant(ev: {
  date?: unknown; time?: unknown; timezone?: unknown; endDayOffset?: unknown; endTime?: unknown;
}, fallbackZone = 'UTC'): number | null {
  if (!isValidTime(ev.time) || !isValidTime(ev.endTime)) return null;
  const span = spanOf(ev);
  if (!span) return null;
 ...
 */
export function spanProblem(ev: {
  time?: unknown; endDayOffset?: unknown; endTime?: unknown;
}): 'offset' | 'end-without-start' | 'ends-before-start' | null {
  if (ev.endDayOffset !== undefined && ev.endDayOffset !== null && !isValidDayOffset(ev.endDayOffset)) return 'offset';
  const hasEnd = ev.endTime !== undefined && ev.endTime !== null && ev.endTime !== '';
  if (hasEnd && !isValidTime(ev.endTime)) return 'ends-before-start';
  if (hasEnd && !isValidTime(ev.time)) return 'end-without-start';
  const offset = isValidDayOffset(ev.endDayOffset) ? ev.endDayOffset : 0;
  if (offset === 0 && isValidTime(ev.time) && isValidTime(ev.endTime) && ev.endTime <= ev.time) return 'ends-before-start';
  return null;
}
 ...
 */
export function endFieldsFor(endDayOffset: unknown, endTime: unknown, hasStartTime: boolean): {
  endDayOffset: number | null; endTime: string | null;
} {
...
export function zoneChoices(current: string): string[] {
  ...
  const out: string[] = [];
  for (const z of [current, ...COMMON_ZONES]) {
    if (!z || seen.has(z) || !isValidZone(z)) continue;
    seen.add(z);
    out.push(z);
  }
  return out;
